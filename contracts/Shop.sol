...
contract Shop {
    ...
    constructor(AddressStorage _tokens) {
        owner = msg.sender;
        tokens = _tokens;
    }
    ...
    */
     ...
     * @param _requests The tuple with items request
     */
    function buyWithToken(Request[] memory _requests, string memory _tokenCode)
        public
        payable
    {
        uint256 allPrice = 0;
        ...
        if (msg.value >= 0) {
            ...
            uint256 balance = IERC20(tokens.getTokenAddress(_tokenCode))
                .balanceOf(msg.sender);
            require(balance >= allPrice, "The token balance is not suffient");

            IERC20(tokens.getTokenAddress(_tokenCode)).transferFrom(
                msg.sender,
                owner,
                allPrice
            );
